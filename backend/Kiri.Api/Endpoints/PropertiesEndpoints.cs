...
public static class PropertiesEndpoints
{
    ...
    private static IResult GetPagedProperties(
    ...
    private static IResult GetPropertyById(int id, IPropertyStorage storage)
    {
        var property = storage.GetById(id);
        return property is null
            ? Results.NotFound(new { Message = $"Property with id {id} was not found." })
            : Results.Ok(property);
    }

    private static IResult CreateProperty(
        ...
        IValidator<PropertyFormData> validator)
    {
        ...
        var createdProperty = storage.Add(formData);
        return Results.Created($"/api/properties/{createdProperty.Id}", createdProperty);
    }

    private static IResult UpdateProperty(
        int id,
        PropertyFormData formData,
        IPropertyStorage storage,
        IValidator<PropertyFormData> validator)
    {
        ...
        var updatedProperty = storage.Update(id, formData);
        return updatedProperty is null
            ? Results.NotFound(new { Message = $"Property with id {id} was not found." })
            : Results.Ok(updatedProperty);
    }

    private static IResult DeleteProperty(int id, IPropertyStorage storage)
    {
        var wasDeleted = storage.Delete(id);
        return wasDeleted
            ? Results.NoContent()
            : Results.NotFound(new { Message = $"Property with id {id} was not found." });
    }

    private static IResult GetStatistics(IPropertyStorage storage)
    {
        var statistics = storage.GetStatistics();
